...
otp_str = str(otp)
previous_email = ""  # Store the previous email

def send_email(receiver_email, otp):
    sender_email = "Enter Your EMail"  # Your email address
    sender_password = "Enter Your Password"  # Your email password
...
def restart_app():
    ...
    email_window.deiconify()  # Show the email entry window again
    email_entry.delete(0, 'end')  # Clear the previous email
    otp_entry.delete(0, 'end')  # Clear the previous OTP

def close_app():
    final_message.destroy()
    email_window.destroy()  # Close the entire app
...
def send_email_and_open_otp_window():
    ...
    if send_email(user_email, otp_str):
        ...
        email_window.withdraw()  # Hide the email entry window
...
# Create the main email entry window
email_window = tk.Tk()
